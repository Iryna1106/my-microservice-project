...
# NEW for lessons 8–9: ECR *push* rights on the nodes.
# The Jenkins Kaniko agent (a pod on these nodes) BUILDS the image and PUSHES it
# to ECR. Kaniko picks up the node's IAM credentials from the EC2 metadata
# service (IMDS), so with this policy attached no extra secret is needed.
# (ReadOnly above only allows pull, not push.)
#
# ⚠️ For a POD to reach IMDS, the node's IMDS "hop limit" must be 2, not the
# AL2023 default of 1 — that is exactly what the launch template below sets.
...
# -----------------------------------------------------------------------------
#  LAUNCH TEMPLATE — lets a POD reach the node's IAM role via IMDS.
#
#  Why this exists: modern EKS nodes use the Amazon Linux 2023 (AL2023) image.
#  On AL2023 managed node groups (with no custom launch template) the IMDS
#  "hop limit" defaults to 1, which means the credentials at 169.254.169.254 are
#  reachable by the host but NOT by pods (a pod is one extra network hop away).
#  So the Jenkins Kaniko pod could not get the node's ECR-push credentials and
#  the image push would fail.
#
#  Raising the hop limit to 2 lets pods reach IMDS. We deliberately set NO
#  image_id and NO instance_type here, so EKS still chooses the AL2023 AMI (from
#  ami_type) and the instance size (from the node group's instance_types).
# -----------------------------------------------------------------------------
resource "aws_launch_template" "node" {
  ...
  metadata_options {
    ...
    http_tokens                 = "required" # keep IMDSv2 enforced (secure)
    http_put_response_hop_limit = 2          # allow pods one extra hop to IMDS
...
resource "aws_eks_node_group" "this" {
  ...
  # Attach the launch template above so nodes come up with the IMDS hop limit
  # that lets the Kaniko pod reach the node's IAM role (to push to ECR).
  launch_template {
